...
class Database{
	...
	public static function Open(){
		...
		Database::$link = mysql_connect(Config::mysql_server, Config::mysql_user, Config::mysql_password);
		if (!Database::$link){
			die("Q".Database::$curQueryNum.": Could not connect: " . mysql_error(Database::$link));
		}
		
		$db_selected = mysql_select_db(Config::mysql_database, Database::$link);
		if(!$db_selected){
			die("Q".Database::$curQueryNum . ": Couldn\"t use ".Config::mysql_database.": " . mysql_error(Database::$link));
		}
	...
	*/
	...
	public static function addPrefix($tableName){
		...
		return Config::mysql_table_prefix . $tableName;
